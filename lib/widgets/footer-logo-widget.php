...
class Zorka_Footer_Logo extends  G5Plus_Widget {
    public function __construct() {
        ...
        $this->widget_description = __( "Logo and sub description", 'zorka' );
        $this->widget_id          = 'zorka-footer-logo';
        $this->widget_name        = __( 'Zorka: Footer Logo', 'zorka' );
        $this->settings           = array(
            'sub_description'  => array(
                'type'  => 'text-area',
                'std'   => '',
                'label' => __( 'Sub Description', 'zorka' )
            )
        );
        parent::__construct();
    }
    ...
    function widget( $args, $instance ) {
        ...
        <div class="footer-logo <?php echo esc_attr($class_custom) ?>">
            <?php if(isset($zoka_footer_logo) && $zoka_footer_logo!='') { ?>
                <a href="<?php echo get_home_url() ?>"><img src="<?php echo esc_url($zoka_footer_logo) ?>" alt="<?php __('Zorka logo','zorka') ?>" /></a>
            <?php } ?>

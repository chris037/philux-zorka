...
class Zorka_Product_Featured_Widget extends  G5Plus_Widget {
    public function __construct() {
        ...
        if(class_exists( 'WooCommerce' )){
            ...
            $this->widget_description = __( "Product Featured Items", 'zorka' );
            $this->widget_id          = 'widget-product-featured-items';
            $this->widget_name        = __( 'Zorka: Product Featured Items', 'zorka' );
            $this->settings           = array(
                'category' => array(
                    'type' => 'select',
                    'std' =>'',
                    'label' => __('Select product from','zorka'),
                    'options' => $categories
                ),
                'per_page' => array(
                    'type' => 'number',
                    'std' =>'4',
                    'label' => __('Per page','zorka')
                )
    ...
    function widget( $args, $instance ) {
        ...
        if ( $products->have_posts() ) : ?>
            ...
            ?>
        <?php else: ?>
            <div class="item-not-found"><?php echo __('No item found','zorka') ?></div>
        <?php endif;

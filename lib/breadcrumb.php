    ...
    function zorka_get_breadcrumb_items() {
        ...
        /* If viewing any type of archive. */
            ...
            if ( is_category() || is_tag() || is_tax() ) {
                ...
                if ( is_day() )
                    $item['last'] = __( 'Archives for ', 'zorka' ) . get_the_time( 'F j, Y' );

                elseif ( is_month() )
                    $item['last'] = __( 'Archives for ', 'zorka' ) . single_month_title( ' ', false );

                elseif ( is_year() )
                    $item['last'] = __( 'Archives for ', 'zorka' ) . get_the_time( 'Y' );
            }

            else if ( is_author() )
                $item['last'] = __( 'Archives by: ', 'zorka' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );

        }

        /* If viewing search results. */
        else if ( is_search() )
            $item['last'] = __( 'Search results for "', 'zorka' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

        /* If viewing a 404 error page. */
        else if ( is_404() )
            $item['last'] = __( 'Page Not Found', 'zorka' );
    ...
    function zorka_breadcrumb_get_bbpress_items() {
        ...
        elseif ( bbp_is_single_topic() ) {
            ...
            if ( bbp_is_topic_split() )
                $item[ 'last' ] = __( 'Split', 'zorka' );

            elseif ( bbp_is_topic_merge() )
                $item[ 'last' ] = __( 'Merge', 'zorka' );

            elseif ( bbp_is_topic_edit() )
                $item[ 'last' ] = __( 'Edit', 'zorka' );
        }

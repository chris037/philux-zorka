...
if ( ! function_exists( 'zorka_wp_title' )) {
    function zorka_wp_title( $title, $sep ) {
        ...
        if ( $paged >= 2 || $page >= 2 ) {
            $title = "$title $sep " . sprintf( __( 'Page %s', 'zorka' ), max( $paged, $page ) );
        }
...
function zorka_header_menu_product_category_filter($arg) {
	...
	return '<li class="x-menu-item-product-category"><span class="pe-7s-menu"></span><span class="product-category-text">' . __('Shop By Categories','zorka') . '</span><ul class="product-category-dropdown">' . wp_kses_post($category_content) .'</ul></li>';
}
...
function zorka_main_menu_args_filter($args){
	...
	<div class="nav-menu-toggle-wrapper">
		<div class="nav-menu-toggle-inner">
			<div class="nav-menu-toggle-icon"> <span></span></div> <span><?php _e('Menu','zorka') ?></span>
		</div>

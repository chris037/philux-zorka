            ...
            <div class="jp-type-playlist">
                <div class="jp-gui jp-interface">
                    <ul class="jp-controls jp-play-pause">
                        <li><a href="javascript:;" class="jp-play" tabindex="1"><i class="fa fa-play"></i> <?php _e( 'play', 'zorka' ); ?></a></li>
                        <li><a href="javascript:;" class="jp-pause" tabindex="1"><i class="fa fa-pause"></i> <?php _e( 'pause', 'zorka' ); ?></a></li>
                    </ul>
                    ...
                    <ul class="jp-controls jp-volume">
                        <li>
                            <a href="javascript:;" class="jp-mute" tabindex="1" title="mute"><i class="fa  fa-volume-up"></i> <?php _e( 'mute', 'zorka' ); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-unmute" tabindex="1" title="unmute"><i class="fa fa-volume-off"></i><?php _e( 'unmute', 'zorka' ); ?></a>
                        </li>
                        <li>
                    ...
                    <ul class="jp-toggles">
                        <li>
                            <a href="javascript:;" class="jp-shuffle" tabindex="1" title="shuffle"><?php _e( 'shuffle', 'zorka' ); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-shuffle-off" tabindex="1" title="shuffle off"><?php _e( 'shuffle off', 'zorka' ); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat" tabindex="1" title="repeat"><?php _e( 'repeat', 'zorka' ); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat-off" tabindex="1" title="repeat off"><?php _e( 'repeat off', 'zorka' ); ?></a>
                        </li>
                    </ul>
                </div>
                ...
                <div class="jp-no-solution">
                    <?php printf( __( '<span>Update Required</span> To play the media you will need to either update your browser to a recent version or update your <a href="%s" target="_blank">Flash plugin</a>.', 'zorka' ), 'http://get.adobe.com/flashplayer/' ); ?>
                </div>
            </div>
        </div>
        <?php
    ...
    function zorka_render_comments( $comment, $args, $depth ) {
        ...
        <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
            ...
            <div class="comment-text">
                ...
                <div class="author">
                    <div class="author-name"><?php printf( __( '%s', 'zorka'), get_comment_author_link() ) ?></div>
                    <div class="comment-meta">
                        <span class="comment-meta-date">
                            <i class="pe-7s-clock"></i> <?php echo  get_comment_date(get_option('date_format'));?>
                        </span>
                        <?php edit_comment_link( __( 'Edit', 'zorka'),'<span class="comment-meta-edit"><i class="pe-7s-hammer"></i>','</span>' ) ?>
                        <?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
                    </div>
                </div>
                <?php if ( $comment->comment_approved == '0' ) : ?>
                    <em><?php _e( 'Your comment is awaiting moderation.', 'zorka' ) ?></em>
               <?php endif; ?>
            </div>
        </div>
    <?php
    ...
    function zorka_comment_form() {
        ...
        $comment_form_args = array(
            ...
            'comment_notes_before' => '<p class="comment-notes">' .
                __('Your email address will not be published.', 'zorka') /* . ( $req ? $required_text : '' ) */ .
                '</p>',
            'comment_notes_after'  => '',
            'id_submit'            => 'btnComment',
            'class_submit'          => 'zorka-button style1 button-md',
            'title_reply'          => __( 'Leave a Comment','zorka' ),
            'title_reply_to'       => __( 'Leave a Comment to %s','zorka' ),
            'cancel_reply_link'    => __('Cancel reply','zorka'),
            'label_submit'         => __('Add Comment','zorka')
        );
...
if (!function_exists('zorka_ajaxComment')) :
    // Method to handle comment submission
    function zorka_ajaxComment($comment_ID, $comment_status) {
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
            ...
            ?>
            <h4 class="comments-title"><?php _e("Comments",'zorka'); ?> (<?php echo esc_html($number); ?>)</h4>
            <ul class="comment-list">
            <?php wp_list_comments(array(
                'style' => 'li',
                'type' => 'comment',
                'callback' => 'zorka_get_list_comments',
                'avatar_size' => 82
            ),$comments); ?>
            </ul>
            <?php zorka_comment_form($comment->comment_post_ID); ?>
            <?php
...
add_action('comment_post', 'zorka_ajaxComment', 20, 2);
    ...
    function zorka_paging_load_more() {
        ...
        if (!empty($link)) :
            ...
            <div class="blog-load-more-wrapper">
                <button data-href="<?php echo esc_url($link); ?>" type="button"  data-loading-text="<i class='fa fa-refresh fa-spin'></i> <?php _e("Loading...",'zorka'); ?>" class="blog-load-more zorka-button button-sm style1" autocomplete="off">
                    <?php _e("Load More",'zorka'); ?>
                </button>
            </div>
        <?php
        endif;
...
if (!function_exists('zorka_post_meta')) :
    function zorka_post_meta() {
        ...
        <span class="entry-meta-comment">
            <?php comments_popup_link( __('<i class="pe-7s-comment"></i> 0 Comment','zorka'),__('<i class="pe-7s-comment"></i> 1 Comment','zorka'),__('<i class="pe-7s-comment"></i> % Comments','zorka')); ?>
        </span>
        <?php endif; ?>

        <?php edit_post_link( __( '<i class="pe-7s-hammer"></i> Edit', 'zorka' ), '<span class="edit-link">', '</span>' ); ?>
    <?php
    }
endif;

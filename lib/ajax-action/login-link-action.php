	...
	<div class="modal-dialog">
		<form id="zorka-popup-login-form" class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="<?php _e('Close','zorka'); ?>"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title"><?php _e('Login','zorka'); ?></h4>
			</div>
			<div class="modal-body">
					<div class="zorka-popup-login-content">
						<div class="form-group">
							<label for="username"><?php _e('Username:','zorka') ?></label>
							<div class="input-icon">
								<input type="text" id="username" class="form-control"  name="username" required="required" placeholder="<?php _e('Username','zorka') ?>">
								<i class="fa fa-user"></i>
							</div>
						</div>
						<div class="form-group">
							<label for="password"><?php _e('Password:','zorka') ?></label>
							<div class="input-icon">
								<input type="password" id="password" name="password" class="form-control" required="required" placeholder="<?php _e('Password','zorka') ?>">
			...
			<div class="modal-footer">
				...
				<div class="modal-footer-left">
					...
					<label for="remember-me" no-value="<?php _e('NO','zorka') ?>" yes-value="<?php _e('YES','zorka') ?>"></label>
					<?php _e('Remember me','zorka') ?>
				</div>
				<div class="modal-footer-right">
					<button type="submit" class="button" style="width: 100%"><?php echo __('Login','zorka'); ?></button>
				</div>
			</div>
		</form>
	</div>
	...
	<div id="zorka-popup-login-wrapper" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
			<form id="zorka-popup-login-form" class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="<?php _e('Close','zorka'); ?>"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title"><?php _e('Sign Up','zorka'); ?></h4>
				</div>
				<div class="modal-body">
					<div class="zorka-popup-login-content">
						<div class="form-group">
						...
						<div class="form-group">
							<label for="email"><?php _e('Email:','zorka') ?></label>
							<div class="input-icon">
								<input type="email" id="email" name="email" class="form-control" required="required" placeholder="<?php _e('Email','zorka') ?>">
								<i class="fa fa-envelope"></i>
							</div>
						</div>
						<div><?php _e('A password will be e-mailed to you','zorka') ?></div>
						<div class="login-message"></div>
					</div>
				</div>
				<div class="modal-footer">
					...
					<div class="modal-footer-right">
						<button type="submit" class="button" style="width: 100%"><?php echo __('Register','zorka'); ?></button>
					</div>
				</div>
			</form>
		</div>
...
function zorka_sign_up_ajax_callback () {
	...
	if ($error['errors']->get_error_code() != '') {
		...
		foreach ($error['errors']->get_error_messages() as $key => $value) {
			$message .= '<div/>' . __('<strong>ERROR:</strong> ','zorka') . esc_html($value) . '</div>';
		}
	}
	else {

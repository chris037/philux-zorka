	...
	<ul class="social-link">
		<?php foreach ( $social_link_fields as $key => $value ): ?>
			<?php if ( isset( $zorka_data[$key] ) && ! empty( $zorka_data[$key] ) ):?>
                <?php if ($value[2] == 1) : ?>
                    <li style="display:none;"><a href="mailto:<?php echo esc_attr($zorka_data[$key]) ?>" target="_top" data-toggle="tooltip" title="<?php echo esc_attr($value[1]) ?>"><i class="<?php echo esc_attr($value[0]) ?>"></i></a></li>
                 <?php else: ?>
                    <li style="display:none;"><a target="_blank" href="<?php echo esc_url($zorka_data[$key]) ?>" data-toggle="tooltip" title="<?php echo esc_attr($value[1]) ?>"><i class="<?php echo esc_attr($value[0]) ?>"></i></a></li>
                <?php endif; ?>
			<?php endif;?>
		<?php endforeach;?>
	</ul>
